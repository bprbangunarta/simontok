                ...
                <div class="card-body text-nowrap">
                    <form action="{{ route('tugas.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="nama_debitur" value="{{ $kredit->nama_debitur }}" readonly>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Nomor HP</label>
                            <input type="text" class="form-control" name="nohp" value="{{ $cif->nohp }}" readonly>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Alamat Rumah</label>
                            <textarea class="form-control" rows="2" name="alamat" readonly>{{ $kredit->alamat }}</textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Nama Petugas</label>
                            <select class="select2 form-select @error('petugas_id') is-invalid @enderror" data-allow-clear="true" name="petugas_id">
                                <option value="">Select</option>
                                @foreach ($petugas as $item)
                                <option value="{{ $item->id }}" {{ $kredit->kode_petugas == $item->kode ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="tab-pane fade active show" id="tabKredit" role="tabpanel">
                        <div class="row" style="margin-top: -15px;">
                            ...
                            <div class="col-md-6 mt-2">
                                ...
                                <input type="text" class="form-control" value="{{ $kredit->tunggakan->tunggakan_pokok }}" readonly>
                            </div>

                            <div class="col-md-6 mt-2">
                                <label class="form-label">Tunggakan Bunga</label>
                                <input type="text" class="form-control" value="{{ $kredit->tunggakan->tunggakan_bunga }}" readonly>
                            </div>

                            <div class="col-md-6 mt-2">
                                <label class="form-label">Tunggakan Denda</label>
                                <input type="text" class="form-control" value="{{ $kredit->tunggakan->tunggakan_denda }}" readonly>
                            </div>
